...
# reading sheet
df=pd.read_csv("Liverpool_2025_26_-_Sample_Stats.csv")
...
plt.title("Liverpool",fontsize=20,fontweight="bold",y=1)
...
plt.xticks(
    ticks=range(5),
    labels=["Salah","Van_Dijk","Alisson","Mac_Allister","Trent"]
 )
...
plt.xticks(
    ticks=range(5),
    labels=["Salah","Nunez","Jota","Diaz","Gakpo"]
)
...
plt.xticks(
   ticks=range(5),
     labels=["van_Dijk","Konate","Jones","Endo","Mac_Allister"]
)
...
plt.xticks(
    ticks=range(5),
    labels=["Salah","Nunez","Jota","Szoboszlai","Diaz"]
)
...
plt.show()
df.to_csv("Liverpool_2025_26_-_Sample_Stats.csv",index=False)
print(df)