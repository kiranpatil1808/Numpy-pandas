...
# reading sheet
df=pd.read_csv("Tottenham_Hotspur_2025_26_-_Sample_Stats.csv")
...
plt.title("Tottenham Hotspur",fontsize=20,fontweight="bold",y=1)
...
plt.xticks(
 ticks=range(5),
 labels=["Son","Vicario","porro","Kulusevski","Romero"],
)
...
plt.xticks(
    ticks=range(5),
     labels=["Son", "Richarlison", "Maddison", "Kulusevski", "Johnson"]
  )
...
plt.xticks(
     ticks=range(5),
     labels=["Romero", "Van_de_Van", "Son", "Lo_Celso", "Sarr"]
 )
...
plt.xticks(
     ticks=range(5),
    labels=["Son", "Maddison", "Kulusevski", "Richarlison", "Johnson"]
 )
...
plt.show()
df.to_csv("Tottenham_Hotspur_2025_26_-_Sample_Stats.csv",index=False)
