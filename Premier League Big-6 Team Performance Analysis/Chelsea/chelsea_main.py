...
# reading sheet
df=pd.read_csv("Chelsea_2025_26_-_Sample_Stats.csv")
...
plt.title("Chelsea",fontsize=20,fontweight="bold",y=1)
...
plt.xticks(
     ticks=range(5),
     labels=["Palmer","caicedo","Petrovic","Enzo","Colwill"]
 )
...
plt.xticks(
     ticks=range(5),
     labels=["Palmer", "Jackson", "nkunku", "Madueke", "Gallagher"]
)
...
plt.xticks(
    ticks=range(5),
    labels=["Badiashile","Gallagher","Palmer","Jackson","Enzo"]
)
...
plt.xticks(
ticks=range(5),
labels=["palmer","Jackson","Nkunku","Madueke","Enzo"]
)
...
plt.show()
df.to_csv("Chelsea_2025_26_-_Sample_Stats.csv",index=False)
